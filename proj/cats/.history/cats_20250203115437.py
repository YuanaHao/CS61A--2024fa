...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == source[i]:
            ...
            break
    print(f"ID: {user_id} Progress: {cnt / len(source)}")
    print(f"{cnt / len(source)}")
    return None
...
def time_per_word(words, timestamps_per_player):
    ...
    times_1 = []  # You may remove this line
    for i in range(len(tpp[0]) + 1):
        times_1.append(tpp[0][i] - tpp[0][i - 1])
    times_2 = []  # You may remove this line
    for i in range(len(tpp[1] + 1)):
        times_2.append(tpp[1][i] - tpp[1][i - 1])
