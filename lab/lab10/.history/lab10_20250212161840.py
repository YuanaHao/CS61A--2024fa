...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        ...
        if operator == 'and':
            ...
            # 检查operator是否是绑定的变量
            if isinstance(operator, str) and operator in bindings:
        ...
        return exp
    elif isinstance(exp, str) and exp in bindings:
        return bindings[exp]
...
def eval_define(expressions):
    ...
    2
    """
    if expressions.rest.first in OPERATORS:  # 检查是否是运算符
        bindings[expressions.first] = OPERATORS[expressions.rest.first]
    elif type(expressions.rest.first) == str or type(expressions.rest.first) == int:
